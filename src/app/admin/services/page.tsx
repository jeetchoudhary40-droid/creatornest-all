...
const STATIC_SERVICES = staticItems
  ...
  .map(item => {
    ...
    return {
      ...
      icon: item.icon?.name || item.icon?.displayName || 'Scissors',
...
export default function AdminServicesPage() {
  const [items, setItems] = useState<any[]>(() => STATIC_SERVICES);
  ...
  const fetchItems = async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (cached) {
        ...
        const servicesOnly = parsed.filter((i: any) => i.item_type === 'service');
    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        ...
        if (items.length === 0) {
          setItems(STATIC_SERVICES);
        }
      }
    } catch (e) {
      console.warn("Failed to fetch services, keeping static fallback", e);
      if (items.length === 0) {
        setItems(STATIC_SERVICES);
      }
    }
  ...
  const handleOpenModal = (item?: any) => {
    ...
    if (item) {
      ...
      setEditingItem(null);
      setFormData({
        title: '', short_desc: '', long_desc: '', category: 'Video Editing', icon: 'Scissors', accent: '#00F2FE', plan: 'free', price: '0', rating: '5.0', thumbnail_url: '', file_url: '', external_url: '', tags: '', features: '',
    ...
    try {
      ...
      if (editingItem) {
        try {
          ...
          console.warn("Offline: Updating service locally", dbErr);
        }
        ...
        try {
          ...
          console.warn("Offline: Inserting service locally", dbErr);
        }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this service?')) {
      try {
        ...
        console.warn("Offline: Deleting service locally", dbErr);
      }
      ...
      <div className="flex justify-between items-center bg-surface border border-white/5 p-6 rounded-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Manage Marketplace Services and Bookings</p>
          </div>
        </div>
        <button 
              ...
              {loading ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-gray-400">
                    <Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" /> Loading services...
                  </td>
                </tr>
              ) : items.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-gray-400">No services found. Create one above!</td>
                </tr>
              ) : (
                items.map(item => (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            ...
            >
              <div className="flex items-center justify-between p-6 border-b border-white/5 sticky top-0 bg-surface z-20">
                <h2 className="text-xl font-bold text-white">{editingItem ? 'Edit Service' : 'Add New Service'}</h2>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Accent Color</label>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Starting Price (₹)</label>
                    <input 
                      type="number" 
                  ...
                  <div className="md:col-span-3">
                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Features / deliverables (One per line)</label>
                    <textarea 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 mt-2">
                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">External Booking Link</label>
                    <input 
                      type="url" 
